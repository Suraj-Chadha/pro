...
class UserTable(Base):
    ...
    def get_json(body):
        if 'username' not in body:
            print("error") 
            ##responseQueueFill({"code":400, "msg":'username is not present'},ch,properties,method)
        if 'password' not in body:
            print("error")
            ##responseQueueFill({"code":400, "msg":'password is not present'},ch,properties,method)
        else:
...
class RideTable(Base):
    ...
    def validateTimestamp(tz):
        try:
            ...
            print("error")
            ##responseQueueFill({"code":400, 'msg':"invalid timestamp %s format. format is DD-MM-YYYY:SS-MM-HH" % (tz)},ch,properties,method)
        return tz
    ...
    def get_json(body):
        if 'destination' not in body:
            print("error")
##responseQueueFill({"code":400, "msg": 'destination not passed in the request'},ch,properties,method)
        if 'timestamp' not in body:
            print("error")
            ##responseQueueFill({"code":400, "msg": 'timestamp not passed in the request'},ch,properties,method)
        if 'created_by' not in body:
            print("error")
            ##responseQueueFill({"code":400, "msg": 'created_by not passed in the request'},ch,properties,method)
        if 'source' not in body:
            print("error")
            ##responseQueueFill({"code":400, "msg": 'source not passed in the request'},ch,properties,method)
        else :
...
def check_content_type(req):
    if not req.is_json:
        print("error")
        ##responseQueueFill({"code": 400, "msg": 'Content-Type unrecognized'},ch,properties,method)

def myconverter(o):
    if isinstance(o, datetime.datime):
        return o.__str__()

engine =  sql.create_engine("sqlite:////app/pdb/riders.db")
