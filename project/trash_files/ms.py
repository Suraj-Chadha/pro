...
def callback1(ch, method, properties, body):
	...
	#-------------------------------------------------------------
		...
		if ret_val is None:
			...
			responseQueueFill(json_body,ch,properties)
	...
	#-------------------------------------------------------------
		...
		try:
			...
			if(ride is None):
			    ...
			    responseQueueFill({"code":200, "action":"ride_info", "response":json.dumps(response , indent=4)},ch,properties)
		except:
	    		responseQueueFill({"code":500,"msg":"Internal Server Error"},ch,properties)
	...
	#-------------------------------------------------------------
	    ...
	    for i in ret_val:
	        response.append(i[1])
	    responseQueueFill({"code":200, "action":"list_all_users", "response":json.dumps(response , indent=4)},ch,properties)
...
#---------------------------------------------DECIDING MASTER SLAVE--------------------------------------------------------

def getLeastPid():
	int leastPid = os.getpid();
	process = [proc for proc in psutil.process_iter() if proc.name == "ms0.py" or proc.name == "ms.py"]
	for i in process:
		print(i.pid,"\n")
		if i.pid < leastPid:
			leastPid = i.pid
...
def callback2(ch, method, properties, body):
	...
	if(action == "write_user"):
		...
		responseQueueFill({"msg": None, "code":201, "action":"write_user"}, ch, properties)
		

	elif(action == "delete_user"):
		UserTable.query_db(body_["username"]).delete_from_db()
		responseQueueFill({"msg": None, "code":200, "action":"delete_user"}, ch, properties)
		...
		responseQueueFill({"response": json.dumps({"ride_id": ride_id}), "code":201, "action":"write_ride"},ch,properties)

	elif(action == "join_ride"):
		RideUsersTable(user_table_name=body_["username"], ride_table_id=body_["ride_id"]).write_to_db()
		responseQueueFill({"msg": None, "code":200, "action":"join_ride"}, ch, properties)

	elif(action == "delete_ride"):
		RideTable.read_from_db(body_['ride_id']).delete_from_db()
		responseQueueFill({"msg": None, "code":200, "action":"delete_ride"}, ch, properties)
...
#GLOBAL_LEAST_PID = getLeastPid()

channel.basic_consume(callback1, 'readQ')
print(' [*] Waiting for -----READ---- messages. To exit press CTRL+C')
channel.start_consuming()

'''channel.basic_consume(callback2, 'writeQ') #,  no_ack=True
print(' [*] Waiting for -----READ---- messages. To exit press CTRL+C')
channel.start_consuming()'''
